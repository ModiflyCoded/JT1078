...
namespace JT1078.Protocol
{
    ...
    {
        private readonly static ConcurrentDictionary<string, JT1078FrameReassembler> livePackageGroup = new(StringComparer.OrdinalIgnoreCase);
        private readonly static ConcurrentDictionary<string, JT1078FrameReassembler> historyPackageGroup = new(StringComparer.OrdinalIgnoreCase);
        public static byte[] Serialize(JT1078Package package, int minBufferSize = 4096)
        {
            ...
            try
            {
                ...
                if (package.Label3.DataType != JT1078DataType.TransparentData)
                {
                    jT1078MessagePackReader.WriteUInt64(package.Timestamp);
                }
                if (package.Label3.DataType != JT1078DataType.TransparentData &&
                    package.Label3.DataType != JT1078DataType.AudioFrame)
                {
        ...
        public static JT1078Package Deserialize(ReadOnlySpan<byte> bytes)
        {
            ...
            if (jT1078Package.Label3.DataType != JT1078DataType.TransparentData)
            {
                jT1078Package.Timestamp = jT1078MessagePackReader.ReadUInt64();
            }
            if (jT1078Package.Label3.DataType != JT1078DataType.TransparentData &&
               jT1078Package.Label3.DataType != JT1078DataType.AudioFrame)
            {
        ...
        public static JT1078Package Merge(JT1078Package jT1078Package, JT808ChannelType channelType)
        {
            ...
            var reassembler = packageGroup.GetOrAdd(cacheKey, _ => new JT1078FrameReassembler());
            return reassembler.Merge(jT1078Package);
        }
        public static byte[] AnalyzeJsonBuffer(ReadOnlySpan<byte> bytes, JsonWriterOptions options = default)
        {
            ...
            using (Utf8JsonWriter writer = new Utf8JsonWriter(memoryStream, options))
            {
                ...
                if (label3.DataType != JT1078DataType.TransparentData &&
                    label3.DataType != JT1078DataType.AudioFrame)
                {
